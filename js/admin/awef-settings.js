...
$window.on('load', function () {
    ...
    jQuery('.widgets-chooser-sidebars').find('li').each(function () {
        ...
        if (/^Page #\d+ - Section #\d+$/.test(thistext)) {
            $thisElem.addClass('awef-chooser-section').hide();
        }
    });
    ...
    if (selected_item == 'template-authors.php') {
        jQuery('#awef-page-template-authors').show();
    }

    jQuery($pageTemplateAttr).change(function () {
        var selected_item = jQuery($pageTemplateAttr).val();
        jQuery('.awef-page-templates-options').hide();
        if (selected_item == 'template-authors.php') {
            jQuery('#awef-page-template-authors').show();
        }
    });
...
function attachAllDynamicSelects() {
    ...
    jQuery('.awef-custom-posts-selector select').select2();
    jQuery('.awef-custom-posts-selector select').select2({
        width: 'resolve',
        ajax: {
            ...
            data: function (params) {
                ...
                return {
                    ...
                    block: activeBlock,
                    action: 'awef_get_posts' // AJAX action for admin-ajax.php
                };
            },
    ...
    jQuery('.awef-custom-posts-selector select').each(function (index) {
...
function attachAllDynamicPostTypes() {
    jQuery('.awef-custom-post-selector input:not([type=hidden])').autocomplete({
...
$doc.ready(function () {
    ...
    jQuery('.awef-toggle-option').each(function () {
        var $thisElement = jQuery(this),
            elementType = $thisElement.attr('type'),
            toggleItems = $thisElement.data('awef-toggle');
    ...
    ------------------------------------------------------------------------------------------ */
    jQuery('#awef-reset-settings').click(function () {
    ...
    if (jQuery('.widgets-php div[id*="figaropost-"]').length) {
        jQuery('#awef-show-sections-sidebars-wrap').show();

        jQuery('.widgets-php div[id*="figaropost-"]').parent().addClass('awef-sidebar-section');
        jQuery('#awef-show-sections-sidebars').change(function () {
            jQuery('.awef-sidebar-section, .awef-chooser-section').toggle();
        });
    }

    /* OPTIONS SEARCH
    ------------------------------------------------------------------------------------------ */
    $searchSettings = jQuery('#awef-panel-search'),
        $searchList = jQuery('#awef-search-list');
    ...
    $searchSettings.on('keyup', function () {
        ...
        if (valThis == '') {
            ...
            jQuery('.awef-label').each(function () {
                var $thisElem = jQuery(this),
                    thistext = $thisElem.text();
    ...
    $searchList.on('click', 'a', function () {
        ...
        jQuery('.awef-panel-tabs ul li').removeClass('active');
        jQuery('.awef-panel-tabs').find('.' + tabId).addClass('active');
    ...
    $doc.mouseup(function (e) {
        var container = jQuery('#awef-options-search-wrap');
    ...
    var lastScrollTop = 0,
            $topSaveButton = jQuery('.awef-panel-content'),
            $bottomSaveButton = jQuery('.awef-footer .awef-save-button');
    ...
    $doc.on('click', '#awef-builder-wrapper .awef-options a', function () {
    ...
    var figaroBlocksColorsOptions = {
        change: function (event, ui) {
            ...
            jQuery(this).closest('.block-item').find('.awef-block-head').attr('style', 'background-color: ' + newColor).removeClass('block-head-light block-head-dark').addClass('block-head-' + getContrastColor(newColor));
        },
        clear: function () {
            jQuery(this).closest('.block-item').find('.awef-block-head').attr('style', '').removeClass('block-head-light block-head-dark');
        }
    };
    ...
    var figaroTitleColorsOptions = {
        change: function (event, ui) {
            var newColor = ui.color.toString();
            jQuery(this).closest('.block-item').find('.awef-block-head .block-preview-title').attr('style', 'color: ' + newColor).removeClass('block-head-light block-head-dark').addClass('block-head-' + getContrastColor(newColor));
        },
        clear: function () {
            jQuery(this).closest('.block-item').find('.awef-block-head .block-preview-title').attr('style', '').removeClass('block-head-light block-head-dark');
        }
    };
    ...
    $doc.on('click', '.toggle-section', function () {
        var $thisElement = jQuery(this).closest('.awef-builder-container');
        $thisElement.find('.awef-builder-section-inner').slideToggle('fast');
        $thisElement.toggleClass('awef-section-open');
        return false;
    });
    ...
    $doc.on('click', '.edit-block-icon', function () {
        ...
        $thisElement.find('> .awef-builder-content-area').fadeIn('fast');

        // Disable the Sortable and Draggable if the PopUp is open
        jQuery('#awef-builder-wrapper, .awef-builder-blocks-wrapper').sortable('disable');
    ...
    $doc.on('click', '.blocks-settings-tabs a', function () {
        var $thisButtonTab = jQuery(this),
            $blockContent = $thisButtonTab.closest('.awef-builder-content-area'),
    ...
    $doc.on('click', '#awef_featured_posts_style a', function () {
    ...
    awef_builder_dragdrop();


    /* IMAGE UPLOADER PREVIEW */
    jQuery('.awef-img-path').each(function () {
        awef_image_uploader_trigger(jQuery(this));
    });


    /* Font Uploader */
    jQuery('.awef-font-path').each(function () {
        awef_set_font_uploader(jQuery(this));
    });


    /* CHECKBOXES */
    var checkInputs = Array.prototype.slice.call(document.querySelectorAll('.awef-js-switch'));
    ...
    if (jQuery('li.menu-top.toplevel_page_awef-plugin-options .awef-plugin-update').length) {
        jQuery('li.menu-top.toplevel_page_awef-plugin-options .wp-menu-name').append(' <span class="update-plugins"><span class="update-count">' + tieLang.update + '</span></span>');
    }
    ...
    jQuery('.tab-sortable').each(function () {
        awef_sortable_tabs_trigger(jQuery(this));
    });

    /* Widget Posts order option  */
    jQuery('.awef-posts-order-option').each(function () {
        awef_widget_posts_order(jQuery(this));
    });
    ...
    $doc.on('widget-added', function (event, widgetContainer) {
        ...
        awef_sortable_tabs_trigger($thisTabs);

        // ------
        var $thisOption = widgetContainer.find('.awef-posts-order-option');
        awef_widget_posts_order($thisOption);
    });
    ...
    /* DISMISS NOTICES
    ------------------------------------------------------------------------------------------ */
    $doc.on('click', '.awef-notice .notice-dismiss', function () {

        jQuery('#awef-page-overlay').hide();

        jQuery.ajax({
            url: ajaxurl,
            type: 'post',
            data: {
                pointer: jQuery(this).closest('.awef-notice').attr('id'),
    ...
    ------------------------------------------------------------------------------------------ */
    var $saveAlert = jQuery('#awef-saving-settings');

    jQuery('#awef_form').submit(function () {

        // Check if the import field has a file
        var importSettings = jQuery('#awef-import-file').val();
        if (importSettings.length > 0) {
            return true;
        }

        // Disable all blank fields to reduce the size of the data
        jQuery('form#awef_form input, form#awef_form textarea, form#awef_form select').each(function () {
            if (!jQuery(this).val()) {
        ...
        jQuery('form#awef_form input:disabled, form#awef_form textarea:disabled, form#awef_form select:disabled').attr('disabled', false);
    ...
    /* SAVE SETTINGS ALERT */
    $saveAlert.fadeOut();
    jQuery('.awef-save-button').click(function () {
        $saveAlert.fadeIn();
    });

    /* SETTINGS PANEL
    ------------------------------------------------------------------------------------------ */
    jQuery('.awef-panel, .awef-notice').css({ 'opacity': 1, 'visibility': 'visible' });
    ...
    jQuery('li.awef-tabs:not(.awef-not-tab)').click(function () {
        jQuery('.awef-panel-tabs ul li').removeClass('active');
    ...
    if (jQuery(currentTab).parent('#awef_form').length) {
    ...
    $doc.on('click', '.del-item', function () {
        ...
        if ($thisButton.hasClass('del-section')) {
            var widgets = $thisButton.closest('.parent-item').find('.awef-manage-widgets').data('widgets');
    ...
    $doc.on('click', '.del-img', function () {
        ...
        $img.fadeOut('fast', function () {
            $img.hide();
            $img.closest('.option-item').find('.awef-img-path').attr('value', '');
        });
    });
        ...
        if (customtext.length > 0) {
            ...
            jQuery('#customList').append('\
				<li class="parent-item">\
					<div class="awef-block-head">\
						'+ customtext + '\
						<input name="_awef_hlights_text['+ customnext + ']" type="hidden" value="' + customtext + '" />\
        ...
        if (source_name.length > 0) {
            ...
            if (source_link.length > 0) {
                source_code += '\
							...
							<input name="_awef_post_source['+ source_next + '][url]" type="hidden" value="' + source_link + '" />\
						';
            }
            else {
            ...
            source_code += '\
						<input name="_awef_post_source['+ source_next + '][text]" type="hidden" value="' + source_name + '" />\
						<a class="del-item dashicons dashicons-trash"></a>\
					</div>\
				</li>\
			';
        ...
        if (via_name.length > 0) {
            ...
            if (via_link.length > 0) {
                via_code += '\
							...
							<input name="_awef_post_via['+ via_next + '][url]" type="hidden" value="' + via_link + '" />\
						';
            }
            else {
                via_code += via_name;
            ...
            via_code += '\
						<input name="_awef_post_via['+ via_next + '][text]" type="hidden" value="' + via_name + '" />\
						<a class="del-item dashicons dashicons-trash"></a>\
					</div>\
				</li>\
			';
    ...
    /* VISUAL OPTIONS
    ------------------------------------------------------------------------------------------ */
    jQuery('ul.awef-options').each(function (index) {
        jQuery(this).find('input:checked').parent().addClass('selected');
    });

    $doc.on('click', 'ul.awef-options a', function () {
        var $thisBlock = jQuery(this),
            blockID = $thisBlock.closest('ul.awef-options').attr('id');
    ...
    jQuery('.predefined-skins-options select').change(function () {
        var skin = jQuery(this).val(),
            skin_colors = awef_skins[skin];

        jQuery('#awef-options-tab-styling').find('.figaroColorSelector').val('');
        jQuery('#awef-options-tab-styling').find('.wp-color-result').attr('style', '');
...
------------------------------------------------------------------------------------------ */
function awef_sortable_tabs_trigger($thisTabs) {

    $thisTabs.sortable({
        placeholder: 'awef-state-highlight',
...
/* IMAGE UPLOADER PREVIEW
------------------------------------------------------------------------------------------ */
function awef_image_uploader_trigger($thisElement) {
    ...
    if ($thisElement.hasClass('awef-background-path')) {
        thisElementID = thisElementID.replace('-img', '');
        uploaderTypeStyles = true;
    }

    awef_set_uploader(thisElementID, uploaderTypeStyles);
}
...
------------------------------------------------------------------------------------------ */
function awef_builder_dragdrop() {

    jQuery('#awef-builder-wrapper').sortable({
        placeholder: 'awef-state-highlight awef-state-sections',
        activate: function (event, ui) {

            var $sortableWrap = ui.item,
                outerHeight = $sortableWrap.outerHeight() + 40;

            jQuery('.awef-state-sections').css('min-height', outerHeight);
        },
    });

    jQuery('.tabs_cats').sortable({ placeholder: 'awef-state-highlight' });
    ...
    jQuery('.block-item').draggable({
        ...
        cursor: 'move',
        zIndex: 100,
        connectToSortable: '.awef-builder-blocks-wrapper',
        revert: true,
        revertDuration: 0,
    ...
    jQuery('.awef-builder-blocks-wrapper').sortable({
        placeholder: 'awef-state-highlight',
        items: '> .block-item',
        cursor: 'move',
        distance: 2,
        ...
        receive: function (event, ui) {
            ...
            ui.item.find('[name^=awef_home_cats]').each(function () {
                var newName = jQuery(this).attr('name').replace(/awef_home_cats\[(\w+)\]/g, 'awef_home_cats\[' + sectionID + ']');
                jQuery(this).attr('name', newName);
            });
        },

        activate: function (event, ui) {
            jQuery('.awef-builder-blocks-wrapper').css('min-height', '65px');
            var $sortableWrap = ui.item.closest('.awef-builder-blocks-wrapper'),
                outerHeight = ($sortableWrap.outerHeight() > 0) ? $sortableWrap.outerHeight() + 40 : '65px';

            $sortableWrap.css('min-height', outerHeight);
            jQuery('.awef-builder-container').addClass('awef-block-hover');
        },

        deactivate: function () {
            jQuery('.awef-builder-container').removeClass('awef-block-hover');
            jQuery('.awef-builder-blocks-wrapper').css('min-height', '');
        },
    }).sortable('option', 'connectWith', '.awef-builder-container');
...
/* IMAGE UPLOADER FUNCTIONS
------------------------------------------------------------------------------------------ */
function awef_set_uploader(field, styling) {
    var awef_bg_uploader;

    $doc.on('click', '#upload_' + field + '_button', function (event) {

        event.preventDefault();
        awef_bg_uploader = wp.media.frames.awef_bg_uploader = wp.media({
            title: 'Choose Image',
            library: { type: 'image' },
            button: { text: 'Select' },
            multiple: false
        ...
        awef_bg_uploader.on('select', function () {
            var selection = awef_bg_uploader.state().get('selection');
...
function awef_color_picker() {
    ...
    jQuery('.figaroColorSelector').each(function () {
        ...
        jQuery('<div class="awef-alpha-container"><div class="slider-alpha"></div><div class="transparency"></div></div>').appendTo($control.parents('.wp-picker-container'));
